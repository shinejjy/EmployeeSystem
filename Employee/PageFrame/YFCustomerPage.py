...
class YFCustomerFlowUpPage(EditableTable):
    def __init__(self, app, window, show):
        super(YFCustomerFlowUpPage, self).__init__(app, window, show, '记录人', '3研部客户对接表', 0, '序号')


class YFCustomerRecordPage(EditableTable):
    def __init__(self, app, window, show):
        super(YFCustomerRecordPage, self).__init__(app, window, show, '联系人', '3研部客户流水表', 0, '序号')


class AuthorizationPage(EditableTable):
    def __init__(self, app, window, show):
        super(AuthorizationPage, self).__init__(app, window, show, '跟进人', '5授权书总表', 0, '序号')
    ...
    def apply_filter(self, event=None):
        ...
        if selected_value == '显示全部':
            ...
            sql = f"SELECT * FROM [5授权书总表] WHERE {self.search_column} = '{selected_value}'"
